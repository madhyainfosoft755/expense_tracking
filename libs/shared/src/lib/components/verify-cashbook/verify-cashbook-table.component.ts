...
@Component({
  selector: 'lib-verify-cashbook-table',
  templateUrl: './verify-cashbook-table.component.html',
  standalone: true,
  imports: [CommonModule, SelectModule, ButtonGroupModule, ImageModule, DatePickerModule, DrawerModule, ReactiveFormsModule, TableModule, ButtonModule, TagModule, DialogModule, InputTextModule, BreadcrumbModule],
  styles: ``
})
export class VerifyCashbookTableComponent implements OnInit, OnDestroy {
    loading = false;
    response: any;
    ...
    loadUnApprovedCashbook(page=1, params={}){
        this.loading = true;
        this.commonAPIService.UnapprovedCashbookExpense(page, {...params, ordering: 'name'})
        .subscribe(
            {
                next: (res: any) => {
                  this.response = res;
    ...
    onSubmit() {
        ...
        this.commonAPIService.VerifyCashbookExpense({remarks: this.remarksForm.value.remarks}, this.claimId, this.claimStatus)
        .subscribe(
            {
                next: (res: any) => {
                  this.claimId = '';
    ...
    exportToCSV() {
      ...
      this.commonAPIService.UnapprovedCashbookExpense(1, {...this.helperSharedService.removeEmptyTrimmedStrings(this.userFilterForm.value, true, true), export: true})
      .subscribe({
        next: (res: any) => {
          const data = res?.data || [];
          const csvRows: any[] = [];
          ...
          this.helperSharedService.exportToCSV(csvRows, 'unapproved_cashbook');
