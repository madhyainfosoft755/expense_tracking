...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, TableModule, FormsModule, DrawerModule, MultiSelectModule, DatePickerModule, SelectModule, ReactiveFormsModule, DialogModule, InputTextModule, ToggleSwitchModule, TagModule, ConfirmDialogModule, BreadcrumbModule, ButtonModule],
...
export class ProjectExpenseComponent implements OnInit, OnDestroy {
  ...
  loadData(page=1, params={}){
    this.loading= true;
    this.userService.getProjectExpense(page, {...params, order_by: '-date', months: this.selectedMonths.join(','), year: this.selectedYear})
      .subscribe({
        next: (data) => {
          this.data = data;
  ...
  onSubmit() {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        this.userService.addProjectExpense(this.amtReceivedFrom.value)
          .subscribe({
            next: () => {
